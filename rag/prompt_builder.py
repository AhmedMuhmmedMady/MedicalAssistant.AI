...
class PromptBuilder:
    ...
    _PROMPTS = {
        ...
        "structure": {
            "ar": "التزم بالهيكلة التالية نصياً (إلزامي):\n\nAnswer:\n[إجابة طبية نهائية مبنية حصرياً على مزج السياقات المسترجعة]\n\nSupporting Context:\n- [نقطة مسترجعة أولى]\n- [نقطة مسترجعة ثانية]\n\nAdvice:\n[توجيه طبي واضح أو نصيحة بالرجوع لطبيب]\n\nConfidence:\n[Low / Medium / High]",
            "en": "Always respond in this structure (mandatory):\n\nAnswer:\n[Final synthesized medical response based strictly on merged context]\n\nSupporting Context:\n- [First RAG bullet point]\n- [Second RAG bullet point]\n\nAdvice:\n[Medical guidance or consultation advice]\n\nConfidence:\n[Low / Medium / High]"
        },
        "non_medical": {
    ...
    def _build_context_block(self, matches: List[KnowledgeMatch], language: str) -> str:
        ...
        for i, m in enumerate(matches, 1):
            parts.append(f"- Key finding {i} (Q: {m.question} | A: {m.answer})")
        return "\n".join(parts)
